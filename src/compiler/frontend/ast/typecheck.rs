            ...
            Node::VarDeclare { ident, typ, expr } => {
                let typ = typ.clone().map_or_else(
                    || {
                        ...
                        expr.as_mut().map_or(Type::Any, |e| {
                            typeof_expr(e, &mut err, &fn_signatures, &mut globals)
                        })
                    },
                    |a| a.0,
                );
...
fn type_matches_collapse(lhs: &MaybeMutable<Type>, rhs: &MaybeMutable<Type>) -> bool {
    ...
    match (lhs.force_immut(), rhs.force_immut()) {
        ...
        (Type::Array(a, c), Type::Array(b, d)) => {
            type_matches_collapse(
                &MaybeMutable::Immutable(&a.0),
                &MaybeMutable::Immutable(&b.0),
            ) && *c == *d
        },
        ...
        _ => {
            println!(
                "typecheck eq collapse `{}` `{}` (assume false)",
                lhs.force_immut(),
                rhs.force_immut()
            );
            false
...
fn typecheck_node(
    node: &AUntypedNode,
    ast: &mut TypedAst,
    err: &mut Vec<ACompileError>,
    fn_signatures: &HashMap<Identifier, (Vec<Type>, Type)>,
    scope: &mut HashMap<Identifier, Type>,
    ret_type: &Type,
) {
    ...
    match node.0.clone() {
        ...
        Node::VarDeclare {
            ...
            let typ = typ.map_or_else(
                || {
                    expr.as_mut()
                        .map_or(Type::Any, |e| typeof_expr(e, err, fn_signatures, scope))
                },
                |a| a.0,
            );
            ...
            ast.push((
                Node::VarDeclare {
                    ident,
                    typ: Some((typ.clone(), Span::default())),
                    expr: expr.map(|a| (a, typ)),
                },
                node.1.clone(),
            ));
        },
        ...
        Node::Return(ref expr) => {
            let typ = expr.as_ref().map_or(Type::BuiltIn(BuiltInType::Unit), |e| {
                typeof_expr(e, err, fn_signatures, scope)
            });
            ...
            if let Some(ref expr) = expr {
                ...
                for typ in types.iter() {
                    if !type_matches_collapse(typ, &MaybeMutable::Immutable(ret_type)) {
                        err.push((
                            Box::new(TypeCheckError::TypeMismatch {
                                expected: ret_type.clone(),
                                found: typ.force_immut().clone(),
                            }),
                            expr.1.clone(),
                        ));
                    }
...
fn typeof_expr(
    ...
    match &expr.0 {
            ...
            match **op {
                Operator::Index => {
                    ...
                    for l in l_may_mut.iter() {
                        if !type_matches_collapse(
                            l,
                            &MaybeMutable::Immutable(&Type::Slice(Box::new((
                                Type::Any,
                                Span::default(),
                            )))),
                        ) {
                            err.push((
                                Box::new(TypeCheckError::TypeMismatch {
                                    expected: Type::Slice(Box::new((Type::Any, Span::default()))),
                                    found: l.force_immut().clone(),
                                }),
                                lhs.1.clone(),
                            ));
                        }
        ...
        Expr::UnOp { opr, .. } => typeof_expr(opr, err, fn_signatures, scope),
        Expr::Ident(id) => scope.get(id).cloned().unwrap_or_else(|| {
        ...
        Expr::FnCall { id, op } => {
            ...
            if op.len() != func.0.len() {
                err.push((
                    Box::new(TypeCheckError::FnArgCountNotMatch {
                        expected: func.0.len(),
                        found: op.len(),
                    }),
                    expr.1.clone(),
...
fn ref_to_type<'a>(
    expr: &AExpr,
    scope: &'a mut HashMap<Identifier, Type>,
    typ: &'a Type,
) -> Vec<MaybeMutable<'a, Type>> {
    match &expr.0 {
        ...
        Expr::BiOp { lhs, rhs, op } => match &**op {
            Operator::Index => {
                ...
                for i in a.iter_mut() {
                    *i = match i {
                        MaybeMutable::Mutable(Type::Slice(item) | Type::Array(item, _)) => {
                            MaybeMutable::Mutable(unsafe { core::mem::transmute_copy(&&item.0) })
                        },
                        MaybeMutable::Immutable(Type::Slice(item) | Type::Array(item, _)) => {
                            MaybeMutable::Immutable(&item.0)
                        },
